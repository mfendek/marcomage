	...
	class CGame
	{
		...
		public function DetermineAIMove()
		{
			...
			if (count($playable_positions) > 0)
			{
				...
				$max_points = 0;
				$choices = array();
				$ai_config = $this->AIConfig($mydata, $hisdata);
				...
				foreach ($playable_positions as $pos)
				{
					...
					{
						$points = 0;
						$preview = $this->CalculatePreview(SYSTEM_NAME, $pos, $i);
						...
						$this->GameData[SYSTEM_NAME] = clone $mydata_b;
						$this->GameData[$opponent] = clone $hisdata_b;
					}
				}
		...
		private function AIConfig($mydata, $hisdata)
		{
			global $game_config;
