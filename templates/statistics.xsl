      ...
      <xsl:choose>
        ...
        <xsl:when test="$param/current_subsection = 'card_statistics'">
          <xsl:for-each select="$param/card_statistics/*">
            <div class="col-sm-4">
              <div class="skin-text top-level values-list">
                <h4>
                  <xsl:value-of select="name()"/> cards
                </h4>
        ...
        <xsl:when test="$param/current_subsection = 'other_statistics'">
          <div class="col-sm-6">
            <div class="skin-text top-level values-list values-list--plain">
              <h4>Game modes</h4>
              <ol>
                <li>
                  <span><xsl:value-of select="$param/game_modes/hidden"/>%
                  </span>
                  <xsl:text>Hidden cards</xsl:text>
                </li>
                <li>
                  <span><xsl:value-of select="$param/game_modes/friendly"/>%
                  </span>
                  <xsl:text>Friendly play</xsl:text>
                </li>
                <li>
                  <span><xsl:value-of select="$param/game_modes/long"/>%
                  </span>
                  <xsl:text>Long mode</xsl:text>
                </li>
                <li>
                  <span><xsl:value-of select="$param/game_modes/ai"/>%
                  </span>
                  <xsl:text>AI mode</xsl:text>
                </li>
                <li>
                  <span><xsl:value-of select="$param/game_modes/ai_wins"/>%
                  </span>
                  <xsl:text>AI win ratio</xsl:text>
                </li>
                <li>
                  <span><xsl:value-of select="$param/game_modes/challenge"/>%
                  </span>
                  <xsl:text>AI challenge</xsl:text>
                </li>
                <li>
                  <span><xsl:value-of select="$param/game_modes/challenge_wins"/>%
                  </span>
