...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="section[. = 'Concepts']">
          ...
          <div class="col-sm-2">
            <p>
              <xsl:if test="am:dateDiff(modified_at, $param/notification) &lt; 0">
                <xsl:attribute name="class">responsive-table__marked-content</xsl:attribute>
              </xsl:if>
    ...
    <xsl:if test="$param/is_logged_in = 'yes'">
      <div class="filters">
        <!-- lower navigation -->
        <xsl:copy-of select="am:lowerNavigation($param/page_count, $param/current_page, 'concepts', 'Concepts')"/>
      </div>
    </xsl:if>
  ...
  <xsl:template match="section[. = 'Concepts_details']">
    ...
    <div class="skin-text details-form">
      ...
      <p>BB code</p>
      <div>
        <input type="text" name="bb_code" maxlength="64" size="25"
               value="[concept={$param/data/id}]{$param/data/name}[/concept]" title="BB code"/>
      </div>
      <p>Effect</p>
      <div>
