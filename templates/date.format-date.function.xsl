  ...
  <func:function name="date:format-date">
    ...
    <xsl:variable name="formatted">
      <xsl:choose>
        ...
        <xsl:when test="starts-with($date-time, '--')">
          <xsl:value-of
                  select="date:_format-date($format, 'Z', 'NaN', number(substring($date-time, 3, 2)), number(substring($date-time, 6, 2)))"/>
        </xsl:when>
        <xsl:otherwise>
    ...
    <xsl:variable name="formatted">
      <xsl:choose>
        ...
        <xsl:otherwise>
          ...
          <xsl:value-of
                  select="date:_format-date(substring($format, $pattern-length + 1), $timezone, $year, $month, $day, $hour, $minute, $second)"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <func:result select="string($formatted)"/>
