...
<xsl:stylesheet version="1.0"
  ...
  <func:function name="am:dateTime">
    ...
    <xsl:variable name="zone"
                  select="concat('Etc/GMT', str:replace(str:replace(str:replace($timezone, '+', '*'), '-', '+'), '*', '-'))"/>
    <func:result>
      ...
      <xsl:choose>
        <xsl:when test="$current &lt; am:max($pageCount - 1, 0)">
          <a class="button button-icon"
             href="{am:makeUrl($location, $pageType, am:min($current + 1, am:max($pageCount - 1, 0)))}">
            <span class="glyphicon glyphicon-chevron-right"/>
          </a>
        </xsl:when>
        <xsl:otherwise>
    ...
    <xsl:variable name="result">
      ...
      <div class="card {$rarity} {$type} {$bgImage} {$foil}">
        ...
        <div class="card__header">
          ...
          <xsl:choose>
            <xsl:when test="$card/rarity = 'Common'">
              <span class="icon-rarity common">C</span>
            </xsl:when>
            <xsl:when test="$card/rarity = 'Uncommon'">
              <span class="icon-rarity uncommon">U</span>
            </xsl:when>
            <xsl:when test="$card/rarity = 'Rare'">
              <span class="icon-rarity rare">R</span>
            </xsl:when>
          </xsl:choose>
        </div>
        ...
        <xsl:choose>
          <xsl:when test="$card/picture">
            <p class="card__keywords">
              <b>
                <xsl:value-of select="$card/keywords"/>
              </b>
            </p>
          </xsl:when>
          <xsl:otherwise>
  ...
  <func:function name="am:cardFilters">
    ...
    <xsl:variable name="result">
      ...
      <xsl:copy-of select="am:htmlSelectBox('keyword_filter', $keywordFilter, $keywords, $keywordsList, 'keyword filter')"/>
      ...
      <xsl:copy-of select="am:htmlSelectBox('advanced_filter', $advancedFilter, $advanced, '', 'advanced filter')"/>
      ...
      <xsl:copy-of select="am:htmlSelectBox('support_filter', $supportFilter, $support, $keywordsList, 'support keyword filter')"/>

      <!-- creation date filter -->
      <xsl:variable name="created">
        <value name="No created filter" value="none"/>
      </xsl:variable>
      <xsl:copy-of select="am:htmlSelectBox('created_filter', $createdFilter, $created, $createdDates, 'date created filter')"/>

      <!-- modification date filter -->
      <xsl:variable name="modified">
        <value name="No modified filter" value="none"/>
      </xsl:variable>
      <xsl:copy-of select="am:htmlSelectBox('modified_filter', $modifiedFilter, $modified, $modifiedDates, 'date modified filter')"/>

      <!-- card level filter -->
      <xsl:variable name="level">
        <value name="No level filter" value="none"/>
      </xsl:variable>
      <xsl:copy-of select="am:htmlSelectBox('level_filter', $levelFilter, $level, $levelsList, 'level filter')"/>
